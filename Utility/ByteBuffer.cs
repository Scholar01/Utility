...
namespace System
{
    ...
    public class ByteBuffer : IDisposable
    {
        ...
        /// </summary>
        /// <param name="value"></param>
        public void PutToken(byte[] Token)
        {
            PutUShort((ushort)Token.Length);
            Put(Token);
        }
        /// <summary>
        ...
        {
            ushort leng = GetUShort();
            if (leng <= 0)
            {
                throw new ArgumentOutOfRangeException("leng");
            }
            return GetByteArray(leng);

        }
        public byte[] GetToken(bool b)
        {
            ushort leng = GetUShort();
            if (leng <= 0)
            {
